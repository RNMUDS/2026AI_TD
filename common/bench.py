...
B1 実効メモリ帯域: 大きなベクトルの加算（読み 2 + 書き 1 → 3×N×4 byte）
B2 実効演算:   行列積 (N×N)·(N×N) → 慣例で 2N^3 FLOP（厳密には 2N^3 − N^2）
...
# Apple が公表しているメモリ帯域（GB/s）．Max など構成で違うものは上位構成の値．
NOMINAL_BANDWIDTH_GBPS = {
    "Apple M1": 68.25, "Apple M1 Pro": 200, "Apple M1 Max": 400, "Apple M1 Ultra": 800,
...
def nominal_bandwidth_gbps(chip):
    """チップ名（machine_info()["chip"]）から公称帯域を返す．表に無ければ None．"""
    return NOMINAL_BANDWIDTH_GBPS.get((chip or "").strip())
